...
class Sales:
    def __init__(self, m_screen, page_name, title, width, height, grid_rows, grid_cols):
        ...
        self.header_lbl.grid(row=0, column=1, sticky="we")
        ...
        self.sellable_lb, self.sellable_items = TkServ.create_listbox(self.m_screen, "sellable_lb", row=10, column=0, width=35, height=25,
            rowspan=14, columnspan=1, data=ProdServ.get_all_sellable_products(
                DB.products), padx=(10, 25), sticky="e")
        ...
        self.cart_lb, self.cart_items = TkServ.create_listbox(self.m_screen, "cart_lb", row=10, column=1, width=80,height=25,rowspan=14, columnspan=2, data=[])

        # Create buttons to manage the cart
        self.add_btn = tk.Button(self.m_screen, text="Add =>", name="add_to_cart_btn", font=("Arial", 12),bg="lightgreen",
            command=lambda: self.add_to_cart())
        self.add_btn.grid(row=9, column=1, sticky="w")

        self.rem_btn = tk.Button(self.m_screen, text="<= Remove", name="rem_item_from_cart_btn", font=("Arial", 12), bg="coral",
            command=lambda: self.rem_from_cart())
        self.rem_btn.grid(row=9, column=2, sticky="e", padx=(0, 35))

        self.clear_btn = tk.Button(self.m_screen, text="Clear", width=25, name="clear_cart_btn", font=("Arial", 12, "bold"), bg="red", fg="white",
            command=lambda: self.clear_cart())
        self.clear_btn.grid(row=24, column=1, columnspan=2, sticky="we", padx=(0, 35))

        self.sell_btn = tk.Button(self.m_screen, text="Sell", width=25, name="sell_cart_btn", font=("Arial", 12), bg="green", 
    ...
    def add_to_cart(self):
        SalesControls.add_item_to_cart(self.m_screen, self.cart_lb, self.cart_items, self.sellable_lb, self.sellable_items,
            self.multiplier_entry.get())

    def rem_from_cart(self):
        SalesControls.rem_item_from_cart(self.m_screen, self.cart_lb, self.cart_items, self.sellable_lb, self.sellable_items, self.multiplier_entry.get())

    def clear_cart(self):
        SalesControls.clear_cart(self.m_screen, self.cart_lb, self.cart_items, self.sellable_lb)

    def sell(self):
        SalesControls.sell(self.m_screen, self.cart_lb, self.cart_items, self.sellable_lb, self.sellable_items, self.selected_client_var)
