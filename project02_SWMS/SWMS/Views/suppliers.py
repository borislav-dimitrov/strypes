...
import Controls.supplierControls as SupplControls
...
class Suppliers:
    def __init__(self, m_screen, page_name, title, width, height, grid_rows, grid_cols):
        ...
        header_lbl = tk.Label(self.m_screen, name="header_lbl", text="Create/Modify Suppliers",
                              font=("Ariel", 15, "bold"))
        header_lbl.grid(row=0, column=2, columnspan=4, sticky="w")

        # Create Buttons
        new_btn = tk.Button(self.m_screen, name="new_supp_btn", text="New", font=("Ariel", 12),
                            width=25, bg="lightblue", command=lambda: self.new_supplier())
        new_btn.grid(row=3, column=1, sticky="w")
        edit_btn = tk.Button(self.m_screen, name="edit_supp_btn", text="Edit/Delete", font=("Ariel", 12),
                             width=25, bg="lightblue", command=lambda: self.edit_supplier())
        edit_btn.grid(row=3, column=5, sticky="w")
    ...
    def new_supplier(self):
        SupplControls.new_supplier(self.m_screen)

    def edit_supplier(self):
        SupplControls.edit_supplier(self.m_screen)
