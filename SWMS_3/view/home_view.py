...
class HomeView(BaseView):
    def __init__(self, m_screen, page_name, systems: dict, resolution: tuple = (1280, 728), grid_rows=30,
        ...
        # Create GUI
        self.welcome = ttk.Label(self.m_screen, text=f"Welcome, {self._controller._logged_user.name}.",
                                 font=self.heading)
        self.welcome.grid(row=0, column=0, columnspan=2, sticky="we")

        # region Buttons
        # Logout Button
        self.logout_btn = ttk.Button(self.m_screen, text="Logout", command=lambda: self.logout())
        self.logout_btn.grid(row=0, column=self.cols - 4, columnspan=3, sticky="e", padx=(0, 5))

        # Create Buttons
        self.wh_btn = ttk.Button(self.m_screen, text="Warehouses", command=lambda: self._controller.warehouses())
        self.wh_btn.grid(row=9, column=4, rowspan=2, columnspan=2, sticky="nsew")
        self.pur_btn = ttk.Button(self.m_screen, text="Purchases", command=lambda: self._controller.purchases())
        self.pur_btn.grid(row=9, column=10, rowspan=2, columnspan=2, sticky="nsew")
        self.sls_btn = ttk.Button(self.m_screen, text="Sales", command=lambda: self._controller.sales())
        self.sls_btn.grid(row=9, column=16, rowspan=2, columnspan=2, sticky="nsew")
        self.tr_btn = ttk.Button(self.m_screen, text="Transactions", command=lambda: self._controller.transactions())
